...
var load_profile_posts = function(){
    ...
    var append_posts = function(data){
        ...
        data.forEach(function(post){
            ...
            <textarea name="comment" id="comment-${post.id}" data-post-id="${post.id}" class="comment comment-textarea" class="comment" placeholder="Add a comment" autocomplete="off" autocorrect="off"></textarea>
            </div>
            </footer>
            </article>`;
            // setting timeout
      ...
      if(data.code === 1000){
        ...
        if(Object.keys(data.data).length>0){
          data.data.forEach(function(t){
            ...
            <a href="${_base_url}/profile/@${t.username}" class="sidebar-pic uk-border-circle">
            <img src="${t.profilePicture}" alt="">
            </a>
            </div>
            <div class="sidebar-profile-text">
            <div class="sidebar-profile-username">
            <a href="${_base_url}/profile/@${t.username}" class="sidebar-username link">
            ${t.username}
            </a>
            </div>
